...
/*----------------------------------------------------------------------------*/
// Apple's AirPort Express RSA private key is what AirPlay 1 uses in place of
// a per-device key: it both signs the Apple-Challenge (RSA_MODE_AUTH, below)
// and decrypts the AES session key an AirPlay sender sends in "rsaaeskey"
// (RSA_MODE_KEY, below). It is not this project's key to hold or distribute -
// see modules/airplay/README.md ("The RSA key situation") and UPSTREAM.md for
// why it was stripped out of this file rather than kept as upstream shipped
// it. The builder supplies it at the gitignored path
// modules/airplay/secrets/raop_private_key.pem; CMakeLists.txt fails the
// build with an explanatory message if that file is missing while
// CONFIG_AIRPLAY_ENABLE=y, and otherwise embeds it via EMBED_TXTFILES, which
// null-terminates it the same way the string literal it replaces was
// null-terminated (mbedtls_pk_parse_key's PEM path requires that terminator
// to be included in the length it is given).
extern const uint8_t raop_private_key_pem_start[] asm("_binary_raop_private_key_pem_start");
extern const uint8_t raop_private_key_pem_end[]   asm("_binary_raop_private_key_pem_end");
...
static char *rsa_apply(unsigned char *input, int inlen, int *outlen, int mode)
{
	...
	rc = mbedtls_pk_parse_key(&pkctx, raop_private_key_pem_start,
	                          (size_t)(raop_private_key_pem_end - raop_private_key_pem_start),
	                          NULL, 0,
