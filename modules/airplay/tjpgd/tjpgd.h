...
/* Upstream includes LVGL's lv_conf_internal.h here and wraps the whole file
 * in `#if LV_USE_TJPGD`, because this copy came from LVGL's bundle. This
 * module is not LVGL's decoder and must not depend on its configuration - see
 * UPSTREAM.md. Defining the symbol keeps the upstream guard structure exactly
 * as it is, which is a smaller and safer change than deleting a conditional
 * whose matching #endif is a thousand lines away. */
