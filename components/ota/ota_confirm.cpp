...
void (*g_prompt)(bool, const std::string&, const std::string&) = nullptr;
...
ConfirmResult request_confirm(const std::string& peer,
  ...
  if (g_prompt != nullptr) g_prompt(true, peer, version);
  ESP_LOGW(kTag, "update offered by %s (version %s); waiting for the board",
           peer.c_str(), version.empty() ? "unknown" : version.c_str());
